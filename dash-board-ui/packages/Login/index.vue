...
<script>
import { getUUID,msgEncode } from 'dashPackages/js/utils'
...
export default {
  ...
  `, '提示', {
    ...
    getCaptcha (auto) {
      ...
      this.captchaPath = this.$dashboardAxios.wrapUrl(`/sys/captcha?uuid=${uuid}`)
    },
    loginResponseHandler (res) {
      ...
      this.$notify({
        ...
        // 跳转到首页
      this.$router.push({ name: 'pageManage' })
    },
    // 错误处理
    exceptionHandler (e) {
      console.error(e)
    ...
    login () {
      this.$refs.loginForm.validate(valid => {
        ...
        this.loading = true
        this.$dashboardAxios.post(`/sys/login`,{
