...
import BigScreenRun from 'dashPackages/DashboardRun/index.vue'
import Icon from 'dashPackages/assets/images/pageIcon/export'
import { getPageType } from './utils'
import _ from 'lodash'
// import axios from 'axios'
// import { post } from 'dashPackages/js/utils/http'
...
export default {
  ...
  methods: {
    ...
    openCascader (node) {
      ...
      this.$dashboardAxios.post('/dashboard/category/tree', { searchKey: '', typeList: ['catalog'], excludeCategory, sort: false }).then(data => {
    ...
    getDataList () {
      ...
      this.$dashboardAxios.post('/dashboard/category/tree', { searchKey: this.searchKey, sort: this.sort }).then(data => {
    ...
    copyPege (nodeData, node) {
      this.$dashboardAxios.post(`/${nodeData.type}/design/copy/${nodeData.code}`).then(() => {
        this.getDataList()
      })
    },
    // 新增仪表盘或新增页面，从空白新建
    ...
    useIt (pageTemplateId, parentNode, type) {
      ...
      this.$dashboardAxios.post(`/dashboard/${type}/design/add/template`, {
        pageTemplateId,
    ...
    addCatalog () {
      this.$refs.form.validate(async (valid) => {
        ...
        if (this.isAdd) {
          this.$dashboardAxios.post('/dashboard/category/add',
            {
              ...this.catalogData,
              id: '',
              code: ''
          ...
          this.$dashboardAxios.post('/dashboard/category/update', { ...this.catalogData, excludeCategory: this.catalogData.code }).then(data => {
            this.catalogVisible = false
            this.getDataList()
          }).catch(() => {
          })
    ...
    deletePageDesign (nodeData, node) {
      ...
      this.$confirm('确定删除该页面设计？', '提示', {
        ...
        this.$dashboardAxios.post(url).then(() => {
          this.$message({
            type: 'success',
            message: '删除成功'
          })
