package com.gccloud.dashboard.core.module.login.controller;
...
public class SysLoginController {
    ...
    public R<SysTokenVO> login(@ApiParam(name = "登录参数", value = "传入json格式", required = true) @RequestBody SysLoginDTO loginDTO) {
        if (dashboardConfig.getCaptcha().isEnable()) {
    ...
    public R<Void> captcha(HttpServletResponse response, @RequestParam("uuid") String uuid) throws IOException {
        ...
        int type = dashboardConfig.getCaptcha().getType();
        int width = dashboardConfig.getCaptcha().getWidth();
        int height = dashboardConfig.getCaptcha().getHeight();
        int length = dashboardConfig.getCaptcha().getLength();
        switch (type) {
            case DashboardConst.Captcha.Type.GIF:
                captcha = new GifCaptcha(width, height, length);
                break;
            case DashboardConst.Captcha.Type.CHINESE_GIF:
                captcha = new ChineseGifCaptcha(width, height, length);
                break;
            case DashboardConst.Captcha.Type.CHINESE:
                captcha = new ChineseCaptcha(width, height, length);
                break;
            case DashboardConst.Captcha.Type.ARITHMETIC:
                captcha = new ArithmeticCaptcha(width, height);
                ((ArithmeticCaptcha) captcha).getArithmeticString();
                break;
            case DashboardConst.Captcha.Type.CHARACTER:
            default:
